...
int main()
{
    ...
    pr_info("enabling PMODBTN SM...");
    sancus_enable(&pmodbtn);
    pr_sm_info(&pmodbtn);

    pr_info("enabling PMODLED SM...");
    sancus_enable(&pmodled);
    pr_sm_info(&pmodled);
    ...
    while (1)
    {
        ...
        if ( (can_id != CAN_ATTEST_ID) )
            pr_info( "WARNING: unexpected CAN button request message format" );

        /* ---------------------------------------------------------------------- */
        pr_info("calling into foo SM for MAC over attestation nonce...");
        ASSERT( !attest_foo(msg, can_msg_len, &btn, mac) );
